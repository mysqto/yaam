...
fn parse(
    ...
    //
    // After attribution, not before it: the grant is a property of the socket, so a record claiming
    // an agent this socket does not belong to has to be turned away as an impersonation rather than
    // answered with a permission decision about somebody else. That is also the invariant the
    // store's own class gate is asserted against — a record naming another agent is refused before
    // its class is considered anywhere.
