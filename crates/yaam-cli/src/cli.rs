...
///
/// Three answers, in three files, none of which is on this command line. Two of them are grants
/// about the caller: the sidecar refuses to forward a subject-derived record from a caller its
/// configuration does not list as one that files them, and the service refuses one from a
/// credential its keyring does not mark the same way. The third is not about the caller at all —
/// [`yaam_core::subject_writes`] refuses the class in any store whose `spec/subject-writes.yaml`
/// does not declare `writes: enabled`, and that file is absent until an operator writes it.
///
/// Installing this binary on a host therefore does nothing on its own, and granting it everything
/// still does nothing: an agent that found it, or that wrote the same JSON to its own socket by
/// hand, is refused at the sidecar, refused again at the service, and refused a third time by the
/// store even with both grants in place. Every refusal is permanent and named, no record is
/// written, and no key is minted — which is the half that could not have been undone.
#[derive(Debug, Parser)]
