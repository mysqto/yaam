...
/// The restore drill, as an operator would run it: two commands, two stores, and a query at the
/// end.
///
/// In here rather than only in the library because the thing being drilled is a *procedure*. What it
/// asserts is the same in both places, and it is the one that matters: the key store never travels,
/// and what does travel comes back answering questions.
#[test]
